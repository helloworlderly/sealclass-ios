...
NSString *const BASE_URL = @"https://api-sealclass.rongcloud.cn/api/v1";
...
+ (AFHTTPSessionManager *)sharedHTTPManager {
    ...
    dispatch_once(&once, ^{
        ...
        manager.completionQueue = dispatch_queue_create("cn.rongcloud.sealclass.httpqueue", DISPATCH_QUEUE_SERIAL);
...
+ (HTTPResult *)httpFailureResult:(NSURLSessionDataTask *)task {
    ...
    result.detail = NSLocalizedStringFromTable(@"HTTPFailure", @"SealClass", nil);
    NSLog(@"%@, {%@}", task.currentRequest.URL, result);
    return result;
}
